...
export class RenderingEngine {
	...
	getRowBounds(scrollY: number, canvasHeight: number) {
		...
		for (let r = 1; r <= this.rowNo; r++) {
			...
			const rowHeight =
				this._rowState.getData?.[r]?.height ?? this.cellHeight;
		...
		for (let r = startRow; r <= this.rowNo; r++) {
			const rowHeight =
				this._rowState._rowDataCache?.[r]?.height ?? this.cellHeight;
			visibleY += rowHeight;
			endRow = r;
	...
	getColBounds(scrollX: number, canvasWidth: number) {
		...
		for (let c = 1; c <= this.columnNo; c++) {
			...
			const colWidth =
				this._colState._colDataCache?.[c]?.width ?? this.cellWidth;
		...
		return { startCol, totalX: totalX + this.leftHeaderWidth , endCol };
	}
